...
document.querySelector('.root-nav').onclick = function(event) {
  ...
  closeAllSubMenu(event.target.nextElementSibling); // В скобках я передал текущую меню тоесть ту в которой я сейчас нахожусь для того что бы закрылись все остальные
